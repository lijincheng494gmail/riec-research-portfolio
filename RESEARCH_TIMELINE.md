...
## 2. Cross-domain applications and failure discovery
...
RIEC-Core separated protocol identity, evidence qualification, non-compensatory gates, actions and claim boundaries. Its v1.0 development boundary was closed before Neuro lockboxes were opened, preventing neural outcomes from being used to retune the frozen Core.

## 5. Distributed adaptive search

RIEC-Agent asked a new question: if many agents each obey a local rule but share finite evidence across claims, aliases and rounds, does local compliance compose into global validity? The benchmark identified a tested regime where it did not and evaluated persistent identity-aware accounting as a response.
